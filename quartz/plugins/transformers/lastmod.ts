...
function coerceDate(fp: string, d: any, debug: (message: string, ...args: any[]) => void): Date {
  ...
  if (invalidDate && d !== undefined) {
    ...
    debug(chalk.green(`  ↳ Successfully parsed using default JavaScript parsing: ${dt.toISOString()}`));
  }
            ...
            for (const source of opts.priority) {
              ...
              if (source === "filesystem") {
                ...
                try {
                  ...
                  debug(chalk.cyan(`  Using repository: ${isInContent ? "content submodule" : "main"}`));
                  ...
                  const gitModified = await activeRepo.getFileLatestModifiedDateAsync(repoRelativePath);
