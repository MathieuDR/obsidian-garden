// Override pageType: render FOLDER pages as the timeline "leaf" view (v4 behaviour),
// filtered to the notes inside that folder. priority 20 wins over folder-page (10).
...
const FolderTimeline = (opts) => ({
  name: "FolderTimeline",
  priority: 20,
  match: ({ slug }) => slug.endsWith("/index") && slug !== "index" && !slug.startsWith("tags/"),
  layout: "folder",
  generate: ({ content }) => {
    const folders = new Set()
    for (const entry of content || []) {
      const slug = entry && entry[1] && entry[1].data && entry[1].data.slug
      if (!slug || String(slug).endsWith("/index")) continue
    ...
    return [...folders].map((f) => {
      ...
      return { slug: f + "/index", title, data: { frontmatter: { title } } }
    })
  },
  body: () => {
    const Component = ({ fileData, allFiles, cfg }) => {
      const locale = cfg?.locale ?? "en-GB"
      const prefix = String((fileData && fileData.slug) || "").replace(/\/index$/, "") + "/"
      const events = []
      for (const f of allFiles || []) {
        if (!isRealNote(f) || !String(f.slug).startsWith(prefix)) continue
